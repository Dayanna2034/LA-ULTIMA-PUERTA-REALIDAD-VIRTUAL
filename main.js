...
const geometryPlano1 = new THREE.BoxGeometry(1.32, 0.63, 1);
const materialPlano1 = new THREE.MeshBasicMaterial({ color: 0xffffff, map:texture });
const Plano1 = new THREE.Mesh(geometryPlano1, materialPlano1);
Plano1.position.z = -0.9;
camera.add(Plano1);
scene.add(camera);

// Lógica para verificar el botón L2 (comúnmente el botón 6 en la mayoría de mandos)
let planoEliminado = false; // Para que se elimine solo una vez
...
function checkGamepadInput() {
  if (gamepadIndex !== null) {
    ...
    if (gamepad) {
      ...
      // Ocultar o mostrar el arma con L2
      if (L2.pressed) {
        Arma.visible = false;
      } else {
        Arma.visible = true;
      ...
      // Eliminar el plano1 con R2 solo una vez
      if (R2.pressed && !planoEliminado) {
        camera.remove(Plano1); // O usa scene.remove(Plano1) si estaba en la escena
        planoEliminado = true;
        console.log("Plano1 eliminado al presionar R2");
      }
    }
  }

  requestAnimationFrame(checkGamepadInput); // Mantiene el bucle activo
}
